...
public class Tree {
    ...
    public boolean isNamePyramid() {
        ...
        if (left != null && this.name.length() > left.getName().length()) {
                return false;
        }

        if (right != null && this.name.length() > right.getName().length()) {
            return false;
        }


        boolean isLeftPyramid = (left == null) || left.isNamePyramid();
        boolean isRightPyramid = (right == null) || right.isNamePyramid();


        return isLeftPyramid && isRightPyramid;
    }
